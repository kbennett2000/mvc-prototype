...
| CMS | TinaCMS (git-based) | No database, visual editor, commits directly to GitHub |
...
/tina                     TinaCMS configuration
  config.ts               Schema — all collections, their fields, and CMS behaviour
  __generated__/          Auto-generated GraphQL schema (do not edit)
  tina-lock.json          Lockfile for Tina Cloud sync (do not edit)

/public                   Static assets
  admin/                  TinaCMS admin app (generated by `tinacms build` — do not edit)
  images/
...
- **Server Actions** — form posts (contact, prayer requests) go to Resend for email delivery; no server-side state mutation.
...
- **Easy CMS preview** — TinaCMS's visual editor works because Next.js builds are deterministic.
...
The CMS configuration at `tina/config.ts` declares the same fields the TypeScript loader expects. **The two are coupled.** If you add a field to one, add it to the other (and to the TypeScript type).
...
The CMS admin app lives at `/admin/` and is built by `tinacms build` as part of the normal build step. It runs entirely in the browser. When an editor saves a change:

1. TinaCMS authenticates the editor via Tina Cloud.
2. TinaCMS commits the changed file directly to the configured branch on GitHub (default: `main`).
3. GitHub fires a webhook to Vercel.
4. Vercel runs `npm run build` and deploys.
5. The new content is live — typically within 1–3 minutes.

There is no intermediate PR review step. Editors' saves go live once the build completes.

The CMS schema — which collections exist, what fields they have, how filenames are generated — is defined entirely in `tina/config.ts`. When you add or change a field there, TinaCMS picks it up on the next `npm run cms` or `tinacms build`. The TypeScript type in `lib/<type>.ts` and the loader in `content/<type>.ts` must be updated to match (same three-way coupling as before; only the config location changed).
